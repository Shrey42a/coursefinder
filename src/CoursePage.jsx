...
function CoursePage(){
    ...
    return(
        ...
        {storedData.map((key) => (
            <div key={key.id} className="rounded-sm bdb overflow-hidden shadow-lg shadow-gray-500 flex flex-col space-y-2 w-80">
             <a className="w-full h-1/2" href={key.link} target="+blank"><img src={key.image} className="imgg w-full cursor-pointer hover:scale-125 transition ease-in-out h-full rounded-t-md rounded-b-sm" /></a>
